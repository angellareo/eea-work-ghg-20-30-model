...
def dump_csv(table, outdir, outfilename):
    ...
    csvout.writerows( list(map(utf8ize, table)) )
    stream.close()
...
def dump_yaml(tables, outdir, outfilename):
    stream = open(os.path.join(outdir, outfilename+'.yaml'), 'w')
    yaml.dump(tables, stream, allow_unicode=True)    
    stream.close()    
...
re_excelfilename = re.compile(r'(\.xls)$')
outfilebase = re_excelfilename.sub('', infilebase)
SimulationData=XLSLoader(infilebase, tables)
dump_yaml(SimulationData.getData(),outdir, outfilebase)